...
def run_full_model(alpha,beta_A,beta_P,delta,epsilon,gamma,zeta,nu,mu,mu_star,sigma):
    '''Defines a model wrapper based on the parameter space in main()'''
    # Length to run each model
    tstart = 0
    tstop = 10
...
def main(N, filename, reduced, pool=None):
    ...
    if reduced:
        problem = {
            ...
            'bounds': [[0.02,0.2], [0.0001,0.01], [0,1], [0.8,8], [0.2,2], [0,1], 
        ...
        problem = {
            ...
            'bounds': [[0.02,0.2], [0.0001,0.01], [0.0001,0.01], [0,1], [0.8,8], [0.00235,0.0235],
                       [0.2,2], [0,1], [0.0023,0.023], [0.00365,0.0365], [0,1]]
            # 'bounds': [[0.003,0.3], [0.00036,0.036], [0.00036,0.036], [0,1], [0.1,10], [0.000744,0.0744],
            #            [0.05,5], [0,1], [0.0007288,0.07288], [0.001155,0.1155], [0,1]]
        }
...
def plot_S1_ST_tbl_from_store(store, show=True):
    ...
    cell_text = [['.02-.2'], ['0.0001,0.01'], ['0.0001,0.01'], ['0-1'], ['.8-8'], ['.00235-.0235'],
                 ['.2-2'], ['0-1'], ['.0023-.023'], ['.00365-.0365'], ['0-1']]
    # alpha, beta_A, delta, epsilon, 
    # zeta, nu, mu, mu*, sigma
    # cell_text = [['0.02-0.2'], ['0.0001,0.01'], ['0-1'], ['.8-8'],
    #             ['0.2-2'], ['0-1'], ['.0023-.023'], ['.00365-.0365'], ['0-1']]
