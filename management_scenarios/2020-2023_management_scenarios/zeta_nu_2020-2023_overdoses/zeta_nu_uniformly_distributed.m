...
for x=1:N;
 ...
 figure(1);
 plot((vec_matrix(:,10)-0.198182427387906)*100/0.198182427387906,(1.359799130226335e-04-J_lhs(1,:))*100/1.359799130226335e-04,'LineWidth',2) 
 hold on
 plot((vec_matrix(:,10)-0.198182427387906)*100/0.198182427387906,(0.001334252863060-K_lhs(1,:))*100/0.001334252863060,'LineWidth',2)
 ...
 figure(2);
 plot(vec_matrix(:,10),J_lhs(1,:),'LineWidth',2) 
 ...
 %Would need to edit to include change in nu, as well
 figure(3);
 plot(vec_matrix(:,10),K_lhs(1,:),'-r','LineWidth',2)
 %set ( gca, 'xdir', 'reverse' )
 xlabel('\zeta')
 ...
 disp('value of total A overdoses from 2020-2023 with these zeta and nu values')
 J_lhs(1,entry)
 disp('percent decrease of total A overdoses from 2020-2023 with these zeta and nu values')
 (1.359799130226335e-04-J_lhs(1,entry))*100/1.359799130226335e-04
 
 disp('value of total H overdoses from 2020-2023 with these zeta and nu values')
 K_lhs(1,entry)
 disp('percent decrease of total H overdoses from 2020-2023 with these zeta and nu values')
