...
epsilon_vec=linspace(epsilon,2*epsilon,N);
...
for x=1:N;
    ...
    %using ode45 because of issues with ode15s (so doesn't match but okay
    %because close enough) per 12/17/2019 meeting--here, essentially
    %monotonic because of size of A so I think either is fine
 ...
 figure(1);
 plot((vec_matrix(:,4)-2.52786996559537)*100/2.52786996559537,(1.359799130226335e-04-J_lhs(1,:))*100/1.359799130226335e-04,'LineWidth',2) 
 hold on
 plot((vec_matrix(:,4)-2.52786996559537)*100/2.52786996559537,(0.001334252863060-K_lhs(1,:))*100/0.001334252863060,'LineWidth',2)
 xlabel('Percent increase in \epsilon')
 ...
 figure(2);
 plot(vec_matrix(:,4),J_lhs(1,:),'LineWidth',2) 
 ...
 %Would need to edit to include change in nu, as well
 figure(3);
 plot(vec_matrix(:,4),K_lhs(1,:),'-r','LineWidth',2)
 %set ( gca, 'xdir', 'reverse' )
 xlabel('\epsilon')
 ...
 disp('percent increase of epsilon')
 (vec_matrix(entry,4)-2.52786996559537)*100/2.52786996559537
 disp('value of epsilon')
 epsilon_vec(1,entry)
 
 disp('value of A with this epsilon value')
 A_lhs(1,entry)
 disp('percent decrease of A from baseline 2023 value with this new epsilon value')
 (0.001681923193256-A_lhs(1,entry))*100/0.001681923193256
 
 disp('value of H with this new epsilon value')
 H_lhs(1,entry)
 disp('percent decrease of H from baseline 2023 value with this new epsilon value')
 (0.013796340958938-H_lhs(1,entry))*100/0.013796340958938
 
 disp('value of total A overdoses from 2020-2023 with this new epsilon value')
 J_lhs(1,entry)
 disp('percent decrease of total A overdoses from 2020-2023 with this new epsilon value')
 (1.359799130226335e-04-J_lhs(1,entry))*100/1.359799130226335e-04
 
 disp('value of total H overdoses from 2020-2023 with this new epsilon value')
 K_lhs(1,entry)
 disp('percent decrease of total H overdoses from 2020-2023 with this new epsilon value')
