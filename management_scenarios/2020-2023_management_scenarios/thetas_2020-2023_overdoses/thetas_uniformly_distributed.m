...
%whatever entry of each vector I am interested in (anywhere from 1 to
 ...
 A_lhs(1,entry)
 disp('percent increase of A from baseline 2023 value with these theta values')
 (A_lhs(1,entry)-0.001681923193256)*100/0.001681923193256
 
 disp('value of H with these theta values')
 H_lhs(1,entry)
 disp('percent decrease of H from baseline 2023 value with these theta values')
 (0.013796340958938-H_lhs(1,entry))*100/0.013796340958938
 
 disp('value of total A overdoses from 2020-2023 with these theta values')
 J_lhs(1,entry)
 disp('percent increase of total A overdoses from 2020-2023 with these theta values')
 (J_lhs(1,entry)-1.359799130226335e-04)*100/1.359799130226335e-04
 
 disp('value of total H overdoses from 2020-2023 with these theta values')
 K_lhs(1,entry)
 disp('percent decrease of total H overdoses from 2020-2023 with these theta values')
