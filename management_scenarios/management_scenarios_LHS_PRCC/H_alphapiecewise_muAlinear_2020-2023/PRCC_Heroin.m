...
function [prcc sign sign_label]=PRCC_Heroin(LHSmatrix,Y,s,PRCC_var,alpha);
...
for rr=1:length(s)
    ...
    %% Plots of PRCCs and their p-values
     %subplot(1,1,1),bar(PRCCs(rr,:)),title(c1);set(gca,'XTickLabel',PRCC_var,'XTick',[1:k]),title('PRCCs'),xlabel('Parameters'),ylabel('PRCC Values');
   
%     subplot(1,2,2),bar(uncorrected_sign(rr,:)),...
