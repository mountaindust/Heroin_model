...
$R_0$ is then calculated as the spectral radius of $FV^{-1}$:
\begin{center}
$R_0$= $\frac{(r+s)+\sqrt{(r-s)^2+4\beta S^* z \nu \sigma_A \sigma_H \zeta}}{2det(V)} $
\end{center}
where $ r=\beta S^* (bc-\sigma_H \nu), s=z(ac-\zeta\sigma_A)$, $a=\zeta +\mu + \mu_A$, $b=\nu + \mu + \mu_H$, $c=\delta + \sigma_A + \sigma_H +\mu$, $z=\theta_1 S^* + \theta_2 P^*$ and $det(V)=(\zeta + \mu +\mu_A)(\nu+\mu+\mu_H)(\delta+\sigma_A+\sigma_H+\mu)-\sigma_A\zeta(\nu+\mu+\mu_H)-\sigma_H\nu$.

We note that the radicand $(r-s)^2+4\beta S^* z \nu \sigma_A \sigma_H \zeta$ is positive, since all parameters are positive. 
