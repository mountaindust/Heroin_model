...
\textcolor{red}{Currently working on details on how my model different than the previous ones.} For the most part, models have not incorporated the connection between prescription opioid misuse and heroin use at all, instead focusing solely on heroin addiction and recovery. Although there are many factors that play a role in population size, susceptibility to drug use, relapse time and other parts of the drug-using process, we take 
...
\textbf{Sensitivity Analysis} \\ %Maybe use Brain Dynamics Toolbox for heat maps? https://github.com/breakspear/bdtoolkit
