...
Dissertation draft \\
Tricia Phillips \\
Before June 19th (first draft) \\

\textcolor{red}{Plan: Need to make changes that made in journal article regarding wording, ordering, etc. (started January 2019) (I've \hl{highlighted things} that were taken out of the journal article but can stay for dissertation...may have missed a few things though, so *read through carefully*)...WOULD PROBABLY BE EASIER to take the journal article and put extra things from here that left out in there instead of re-working this document\\
 Re-order introduction so that most general statistics are at the beginning and then go to specifics: prescription opioids, then heroin, then fentanyl. Other models, then goals. \\
 Can use narrative language and wander a bit more here than in a journal article. \\
 Fix references: cite source after first sentence of mentioning it, and then continue on with details afterward if applicable.\\
 Look around at folders to make sure have put in all information that may not have been put in heroin article (for example, Matlab\_descriptions.pdf regarding more detailed explanation of ode solver, multistart, fmincon, etc.}
...
\hl{Heroin was first formulated in a hospital in 1874, but production on a commercial level did not begin until 1898, when the Bayer Company in Germany marketed and sold the new drug. Prescription heroin was deemed helpful for treating bronchitis, asthma and tuberculosis, but near the turn of the century, the dangers of the drug were starting to be revealed, such as it's potential for addiction. In 1920, the American Medical Association stated that heroin should not be manufactured, sold, or prescribed in the United States, and in combination with the amount of crime resulting from heroin use, a law was passed in 1924 prohibiting crude opium imports with the intention of heroin production. With the start of World War II, traffickers diluted the drug more and more, due to tighter border controls and fewer supplies entering the country} \cite{UnitedNations}. \textcolor{red}{\hl{More heroin history: find reliable source for smuggling of heroin from China to US in 1930s; motor vehicle vs. drug overdose stats; ~750,000 heroin addicts from 1965-1970; purity level/price of heroin; provide argument for why the problem is so large now that we need data-driven models compared to just a theoretical heroin addiction model as done for decades.}}
...
\textcolor{blue}{Question: Maybe leave out since so prescription focused?}: \hl{In 1995, the American Pain Society stated that pain was being under-treated in doctor offices and hospitals, and consequently, created guidelines for the improvement for pain assessment for acute and cancer cases. This included the recording of pain levels such as on a vital signs sheet} \cite{Mandell, APSQCC}. \hl{In 2000, the Joint Commission required physicians to accept and respect the self-reporting of pain by patients. By the early 2000s, drug manufacturers funded publications and physicians to support opioid use for pain control}\cite{Mandell}.  
...
\hl{The White and Comiskey model was modified later on by Liu and Zhang in order to incorporate a relapse distribution in which there was a non-constant time to relapse. They formulated a delay-differential equation, and also took away the assumption that the population was constant. Their conclusions about the sensitivity of $\mathscr{R}_0$ to certain parameters were in line with those from White and Comiskey} \cite{Liu}. \hl{Using the same three compartments, another model was formulated with the idea of the relapse rate relying on the length of time the individual has been undergoing treatment. This coupled ODE-PDE model assumed that susceptibles could only become addicted via interaction with heroin users not undergoing treatment. Prevention was deemed more important than treatment, yet again} \cite{Fang1}. \hl{Fang, Li, Martcheva and Caialso (2015) also modeled the heroin epidemic allowing susceptibility of becoming a drug user to depend on age, again resulting in a coupled ODE-PDE model} \cite{Fang2}. \hl{A non-autonomous version of the White and Comiskey model included a distributed time delay for becoming a drug user along with both the parameters and the total high-risk population size being time-dependent} \cite{Samanta}. \hl{From this, an autonomous model was developed incorporating the distributed time delay, which was converted into a discrete model with the time step-size being one} \cite{Abdurahman}. \hl{Finally, a three compartment model consisting of susceptibles, users not in treatment, and those in treatment, assumed that those not in treatment could not recover and return to being susceptible, and that it took two contacts with a drug user to become addicted, resulting in a nonlinear incidence rate; results showed that perhaps the heroin prevalence undergoes periodic changes depending on conditions}\cite{Ma}. 
...
\hl{Will show existence of solution to my ODE system?}
...
\hl{We will now show that starting with non-negative initial conditions, solutions for the system (1)-(5) are guaranteed to be non-negative for all time, using the following theorem which is an adaptation of Theorem 2.1 in} \cite{Smith}: 
 
\hl{\textbf{Theorem 1}} (\cite{Smith}: \hl{adapted version of Theorem 2.1, Chapter 5)}
\begin{comment} / Brittany Stephenson's dissertation 
\end{comment}
\hl{ \textit{Assume that when $\phi \in \mathbb{R}^n$ satisfies $\phi \geq 0$ with $\phi_{i}(0)=0$ for some $i$, then $f_i(\phi) \geq 0$. Then, if $\phi \in \mathbb{R}^n$ satisfies $\phi \geq 0$, the solution of $x'(t)=f(x(t)), x(t_{0})=\phi$ where $f: \mathbb{C}$ $\rightarrow$ $\mathbb{R}^n$ continuous, $\mathbb{C} \subset \mathbb{R}^n$ open and $\phi \in \mathbb{R}^n$, satisfies $x(t) \geq 0$ for all $t \geq t_{0}.$} }

\textcolor{red}{\hl{Include in dissertation draft, cannot highlight}} \\
...
\textcolor{red}{\hl{Include in dissertation draft (if not in paper already), cannot highlight}} \\
...
\end{center}
 ...
 \textcolor{red}{Can now interpret biologically the reproduction number, depending on which eigenvalue it is! Need to prove AFE is stable if $\mathscr{R}_0$ $<$ 1 and unstable if $>$ 1?}
...
-In literature, see if PA pathway from P to A significant at all and similarly RA and RP from R to A \\
-Ideas about supervised injection sites; NOVA Addiction episode regarding individuals overdosing and then being released from the ER with no help with their addiction (need to have a way for immediate medical attention such as offered buprenorphine); social isolation a huge problem; 
...
\begin{center}
\includegraphics[scale=0.45]{Data1_multistart_plot.pdf}
...
\begin{center}
\includegraphics[scale=0.45]{Data4_multistart_plot.pdf}
...
We use AIC (Akaike's Information Criterion) in order to compare the fit of our model to data in both the alpha linear case and alpha piecewise linear case \cite{Motulsky}. The AIC score balances the change in the fit with the change in the number of parameters being estimated. The idea is to determine whether the model fits the data better because of the addition of one more parameter or because it actually is a better fit, i.e. is it a better fit considering we add one more parameter to estimate? The score is meaningless by itself and only has meaning in the context of comparing two scores. The AIC score is the sum of the lack of fit and the complexity of the model and is given by:

$$\text{AIC}=N \ln (\frac{SS}{N})+2K$$
where in our case N is the number of data points the model is fitting to,  
$$\text{SS}= \displaystyle \sum_{i=1}^{5} \text{Diff1(i)}^2  + \displaystyle \sum_{i=1}^{5} \text{Diff2(i)}^2 + \displaystyle \sum_{i=1}^{3} \text{Diff3(i)}^2 + \displaystyle \sum_{i=1}^{24} \text{Diff4(i)}^2,$$
and K is the number of parameters we estimate with non-linear least squares plus one since the value of $SS$ is also being estimated.
...
Note that if N were large, the denominator in this last fraction would be much larger than the numerator and therefore, the corrective term would be very small. 
...
We observe that $\ln(1.2961e-04)-\ln(8.0617e-04)$ will always be negative (adding an additional parameter will reduce the sum of squares value and rewritten as $\ln(\frac{1.2961e-04}{8.0617e-04})$, the argument will always be less than 1) and the rest of the terms deal with the difference of adding one more parameter, which will always be positive. In this this case, however, these positive terms added on is not enough to compensate for the first negative term, and the net change is negative; this means that the difference of the sum of squares values is better than expected from the addition of one estimated parameter.

Since the AIC scores are relatively far apart, it is with confidence that the alpha piecewise linear model is the better model out of the two. To support this choice, the probability that the alpha piecewise linear model is the better of the two can be given by the following calculation:

$$\text{probability}=\frac{e^{-0.5(\text{AIC}_{cP}-\text{AIC}_{cL})}}{1+e^{-0.5(\text{AIC}_{cP}-\text{AIC}_{cL})}}$$
$$=\frac{e^{-0.5(-58.92)}}{1+e^{-0.5(-58.92)}}$$
$$\approx 1.0.$$

This means there is a 100\% chance that the alpha piecewise linear model is a better choice than the alpha linear model for fit to the data with the additional parameter being estimated. (Note that when using $\text{AIC}_{cL}-\text{AIC}_{cP}=58.92$, the probability that alpha linear is better than alpha piecewise is 1.6057745e-13, which rounds to 0). 

The evidence ratio gives information on how many more times likely the alpha piecewise linear model is to being correct compared the alpha linear model. Since our probability is 1 above, we expect this ratio to be large. It is defined as:

$$evidence ratio=\frac{\text{probability alpha piecewise linear model is correct}}{\text{probability alpha linear model is correct}}$$
...
\textcolor{red}{Edit: Comparing the sensitivity analyses between alpha linear and alpha piecewise linear, the main difference is seen in the parameter $m$ (alpha linear case was more sensitive to this parameter because it was the slope of alpha but in the alpha piecewise linear case, m was very very small and c was bigger, and that took the place of the more sensitive slope parameter). In both cases, the first-order index results are very similar to the total-order index results which suggests that there are not higher-order interactions occurring. \\
...
Although a minimum of $N=17+1=18$ sample points for the alpha linear case is required, the sample size must be big enough for which a similar set of parameters are deemed important for a certain output with consecutive runs. In this case, the best we are able to do is with $N=3000$ runs, although the inclusion/exclusion of the importance of $\beta_A$, $\nu$, and $\omega$ may change in subsequent runs. \textcolor{red}{As increase nsample number, more and more become significant, and fewer have the opportunity to bounce between not significant and significant; should there be an upper limit?}

Below are the monotonicity plots for alpha linear for FILL IN samples; we note that although some plots may seem non-monotonic, they are actually flat due to the scale on the values. 

\textcolor{red}{Fill in?}


Below are the monotonicity plots for alpha piecewise linear for FILL IN samples; we note that although some plots may seem non-monotonic, they are actually flat due to the scale on the values. 


\textcolor{red}{Fill in?}


We utilize the same ranges of parameters as shown in Table FILL IN and FILL IN which are 50\% of baseline parameter values. \\

\captionof{table}{LHS/PRCC Sensitivity Analysis Results for alpha linear}
...
\captionof{table}{LHS/PRCC Sensitivity Analysis Results for alpha piecewise linear}
...
{Input} & \shortstack{PRCC for proportion of \\ Hclass at final time} & \shortstack{$p$-value for proportion of \\ H class at final time}  \\ [0.4ex]
 \hline
...
It is beneficial to also look at Sobol sensitivity analysis in order to examine if there are any higher-order interactions among parameters that are occuring and also to be able to apportion the output variance to certain inputs. 
