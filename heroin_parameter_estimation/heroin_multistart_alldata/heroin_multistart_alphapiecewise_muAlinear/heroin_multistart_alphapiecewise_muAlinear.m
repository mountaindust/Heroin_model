%File name: heroin_multistart_alphapiecewise_muAlinear.m 
%see bottom for explanation of how code works 
...
% It is composed of 3 main sections (from top to bottom of code: define optimization problem, formulate objective function to minimize, and state the systems of ODEs) with details described below. 
% 
% 1) I have my ODE system at the very bottom of the code starting with function alpha=a(t,pars). This part defines my time-dependent alpha, time-dependent mu_A, and system of 5 ODEs (as well as additional equations involved with the estimation) to solve. 
% 
% 2) In the first section (starting at the top), I create an optimization problem that uses the optimization function fmincon to find the minimum of my objective function that I call "value=HeroinModel_ODE15s" (which is the sum of the weighted 2-norms for each data type). I give bounds on the parameters and supply a starting point, x_0, as the point in the middle of the ranges for each of the parameters. I run multistart with a total of n starting points (n-1 of which are generated by multistart) to find multiple local solutions to the optimization problem. 
% 
% 3) Going to the second section starting with function value=HeroinModel_ODE15s(z): here, z is the vector of parameters that we want to estimate, each parameter assigned one spot in the vector. These parameters, along with parameters that have assumed values, are all thrown into a vector called pars which is used in the ODEs. The n different sets of parameter values are fed into the z vector one at a time and the ODEs are solved using ode15s for each set of parameters. 
% The solutions for each set of parameters are used in the Estim vectors (that represent model output) which are what are being compared to the data. The differences between the Estim values and the actual Data values are calculated using a weighted 2-norm for each data type and the sum of those weighted 2-norms is = "value" (the objective function, what we'd like to minimize). 
% 
% 4) The set of parameters that minimize the objective function I define as the vector x back in the first section at the top. This optimal parameter solution is used to solve the ODEs and produces plots of the solution curves as well as the fit of the model simulations to data. 
% 
% 
