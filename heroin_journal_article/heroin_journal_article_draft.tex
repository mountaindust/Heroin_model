...
The following table displays the 17 inputs that were estimated with this process, along with their resulting estimated values. The objective function value (using rounded parameters found in Table 4) is fval$=$0.1279. \\
...
We use the AIC (Akaike's Information Criterion) in order to compare between alpha linear and alpha piecewise linear cases. The idea here is to determine whether the model better fits with the data because of the addition of one more parameter or because it actually is a better fit, i.e. is it a better fit considering we add one more parameter to estimate. The score is meaningless by itself and only has meaning in the context of comparing two scores. \textcolor{red}{Fill in.} \\ \\
