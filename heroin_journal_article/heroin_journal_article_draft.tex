...
Thus, we will extrapolate this information to say that the rate that prescription opioid users and opioid addicts move to heroin use is 19 times greater than the rate at which susceptibles move to heroin use (i.e. $\theta_2 + \theta_3$ $>$ 19$\theta_1$). We will make an assumption here that $\theta_2 + \theta_3$ $=$ 19$\theta_1$ and that $\theta_2$ is approximately 3$\theta_1$ \textit{\textcolor{purple}{can't find source, this is just an estimate}}, i.e. there is only a slightly elevated risk of moving to heroin risk when using opioids in a non-addictive way; this leaves $\theta_3=16\theta_1.$ \\


\textcolor{red}{Note: have data points for P for 2013-2017, A for 2014-2015, and H for 2014-2016, so want to start at 2013.}

Given the data presented in Table 1, we approximate the initial conditions of our states for 2013. There were 1,845,144 total individuals who were prescribed opioids (including those addicted) at some point during 2013. %data table 
With a Tennessee average in 2015 of 118.3 prescriptions per 100 people \textcolor{purple}{(keeping looking for 2013)}, we calculate 
$$\frac{1.183 \text{ prescriptions}}{\text{person$\cdot$ year}}\cdot \frac{5,517,176 \text{ people}}{1,845,144 \text{ prescription users}} \approx \frac{ 3.54 \text{ prescriptions}}{\text{prescription user $\cdot$ year }}$$ 
throughout the year. %TN opioid summary
In 2013, the average number of days of supply per prescription at the national level was approximately 16.5 days. %CDC
Thus, this means that prescription opioid users have approximately $$\frac{3.5 \text{ prescriptions}}{\text{prescription user $\cdot$ year}} \cdot \frac{{16.5 \text{ days}}}{\text{prescription}} \approx \frac{57.75 \text{ days}}{\text{prescription user $\cdot$ year}} $$
that prescriptions are being used, which is approximately one sixth of the year. Therefore, we make an estimate that one sixth of the total number of prescription users throughout 2013 were using prescription opioids at the beginning of the year \textit{\textcolor{purple}{better way of thinking about this?}}. This means that $$\text{\textcolor{purple}{not currently correct...}}1,845,144 \text{ prescription users} \cdot \frac{1}{6} \frac{\text{days of prescription use}}{\text{year}} \approx 307,524$$
and taking 307,524 individuals out of the total population in 2013 of 5,517,176 gives 0.056 of the population using opioid prescriptions at the beginning of 2013. Assuming, as before, that 10\% of these prescribed users are addicted, we have that $A_0=0.0056$ and $P_0=0.056-0.0056=0.05.$ %explanation in data table above 
\\ \\
\textcolor{purple}{Fragmenting data together for R: would it be better to use info from 2015 which includes data about number of addicts and just assume the same for 2013 (got 0.058 in recovery, but how make estimate on how many are stably recovered: maybe 10\% are actually in that class?) \\
\\ OR make assumption on number of addicts in 2013 based on ``10\% of prescription users"...but that doesn't match up to 2015 data estimate for opioid addicts.}
\\ \\
Since heroin is such an addictive substance and the only ways to leave the compartment are through dying or becoming stably recovered, we make the assumption that the majority (we make an estimate of 90\%) of individuals using heroin in 2015 \textcolor{purple}{(don't have for year 2013)} are using the drug at the beginning of the year. This results in $$\frac{0.9 \cdot 14,000 \text{ heroin users}}{5,607,117 \text{ individuals in population}} \approx 0.0022$$
which we take to be our value for $H_0.$
