...
\footnotesize
Prescription opioid addicts (includes heroin addicts) & \footnotesize{\textit{48,674}} &  \footnotesize{\textit{48,163}} & \footnotesize{48,000}
...
\footnotesize
Prescribed opioid users (excludes those addicted)   & \footnotesize{\textit{\textcolor{blue}{1,828,096}}} &\footnotesize{\textit{\textcolor{blue}{1,807,287}
}} & \footnotesize{\textcolor{blue}{\textbf{1,802,570}}} & \footnotesize{\textcolor{blue}{\textbf{1,746,478}}} &  \footnotesize{\textit{\textcolor{blue}{1,622,545}
}} &-\\
\footnotesize
...
The total number of prescribed opioid users each year, however, includes individuals addicted to their prescriptions. In a 2015 study done among adults with prescription opioid use disorder, 44.3\% of them had obtained prescription opioids for their most recent episode of misuse from 1 or more physicians \cite{Han}. Therefore, we assume that 44.3 out of 100 addicts are personally prescribed opioids and subtract 44.3\% of the number of prescription opioid addicts in each year from the prescribed opioid class in the corresponding year to obtain the number of non-addicted prescribed opioid users for 2013-2017. We note that although we assume individuals who are addicted to heroin are not using \textit{prescribed} opioids, since they are more expensive and provide less of a high, we do need to subtract 44.3\% of all prescription opioid addicts (including heroin addicts) since the study consisted of individuals with prescription opioid use disorder, regardless of their heroin addiction status. \cite{NIDA1}. \\
...
Similarly, we may calculate $\mu_{H}$. There were 7,560 heroin/fentanyl addicts in 2015 and 374 heroin-related overdoses. We make the simplifying assumption that if an individual died of a heroin overdose they were addicted. This means that (7,560-374)/7,560 $\approx$ 0.951 is the proportion of heroin users that remain at $t=1$, which implies 0.951$H_0=H_0 e^{-\mu_{H}(1)}$, which results in \boldmath{$\mu_{H}$ $\approx 0.0507$} \unboldmath, the continuous-time rate at which individuals are dying from the heroin class.
